create unique index if not exists members_one_per_application_key
  on public.members (organisation_id, membership_application_id)
  where membership_application_id is not null;
...
begin
  ...
  where m.organisation_id = p_organisation_id
    and m.membership_application_id = v_application.id
  ...
  select id
  into v_membership_period_id
  from public.membership_periods mp
  where mp.organisation_id = p_organisation_id
    and mp.member_id = v_member_id
    and mp.status = 'active'
  order by mp.starts_at desc, mp.created_at desc
  limit 1;

  if v_membership_period_id is null then
  ...
  select id
  into v_membership_card_id
  from public.membership_cards mc
  where mc.organisation_id = p_organisation_id
    and mc.member_id = v_member_id
    and mc.card_status = 'active'
  order by mc.issued_at desc, mc.created_at desc
  limit 1;

  if v_membership_card_id is null then
  ...
  values (
    ...
    p_reviewed_by,
    case
      when v_application.status = 'approved' then 'membership_application_approval_reconciled'
      else 'membership_application_approved'
    end,
